        ...
        checkApple (state){
            ...
            if(apple.x === snakeHead.x && apple.y === snakeHead.y) {
                ...
                do {
                    ...
                    isOnSnake = state.snake.find(snakeElem => {
                        return (snakeElem.x === apple.x && snakeElem.y === apple.y)
                    });
                } while (isOnSnake);
...
export const { changeStatus, moveSnake, saveKey, setDirection, checkApple, checkGameOut } = gameSlice.actions
